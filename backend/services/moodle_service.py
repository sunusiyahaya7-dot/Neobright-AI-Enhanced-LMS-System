...
class MoodleService:
    ...
    def get_quizzes_by_course(course_id: int):
        """
        Fetch quiz information for a course.
        
        Uses Moodle mod_quiz_get_quizzes_by_courses.
        
        Args:
            course_id: Moodle course ID
        
        Returns:
            Dict with quiz info: {'quizzes': [...], 'warnings': [...]}
        """
        try:
    ...
    @staticmethod
    def get_quiz_access_information(quiz_id: int, moodle_user_id: int):
        """
        Get access information for a quiz (can student access it, etc).
        
        Uses Moodle mod_quiz_get_quiz_access_information.
        
        Args:
            quiz_id: Moodle quiz ID (instance ID, not cmid)
            moodle_user_id: Moodle user ID (student)
        
        Returns:
            Dict with access info and warnings
        """
        try:
            print(f"Fetching quiz access info for quiz {quiz_id}, user {moodle_user_id}...")
            ...
            print(f"Error fetching quiz access info for quiz {quiz_id}: {e}")
            import traceback
            traceback.print_exc()
            # Return safe defaults
            return {"canstudentreview": False, "canreviewresponses": False}

    @staticmethod
    def get_quiz_user_attempts(quiz_id: int, moodle_user_id: int):
        """
        Get student's attempts on a quiz.
        
        Uses Moodle mod_quiz_get_user_attempts.
        ...
        Returns:
            Dict with attempts and warnings: {'attempts': [...], 'warnings': [...]}
        """
        try:
            print(f"Fetching quiz attempts for quiz {quiz_id}, user {moodle_user_id}...")
            url = MoodleService._build_url("mod_quiz_get_user_attempts")
            params = {
                "quizid": quiz_id,
                "userid": moodle_user_id,
                "status": "all"  # Get all attempts (finished or in progress)
            }
            ...
            print(f"Error fetching quiz attempts for quiz {quiz_id}: {e}")
            import traceback
            traceback.print_exc()
